      ...
      <div className="space-y-5">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-1">WSJF-ranked features with stories</p>
          </div>
          <Button className="gap-2" onClick={() => { setEditingFeature(null); setFeatureFormOpen(true); }}>
        ...
        <div className="grid grid-cols-4 gap-3">
          <Card><CardContent className="p-4 text-center">
            <div className="text-2xl font-bold">{features.length}</div><div className="text-xs text-muted-foreground">Total Features</div>
          </CardContent></Card>
          <Card><CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-red-400">{mustHave}</div><div className="text-xs text-muted-foreground">Must Have</div>
          </CardContent></Card>
          <Card><CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-amber-400">{inProgress}</div><div className="text-xs text-muted-foreground">In Progress</div>
          </CardContent></Card>
          <Card><CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-emerald-400">{done}</div><div className="text-xs text-muted-foreground">Done</div>
          </CardContent></Card>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="relative flex-1 max-w-sm">
            ...
            <Input placeholder="Search features..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9" />
          </div>
          <span className="text-xs text-muted-foreground">{filtered.length} features • {totalStories} stories</span>
        </div>
            ...
            {filtered.map((f, idx) => {
              ...
              return (
                <div key={f.id} className="rounded-lg border border-zinc-800 overflow-hidden">
                  <div
                    className={cn(
                      "flex items-center gap-3 px-4 py-3 cursor-pointer hover:bg-zinc-900/50 transition-colors",
                  ...
                  >
                    ...
                    <Badge className={cn("text-[10px] shrink-0", statusColors[f.status])}>{f.status}</Badge>
                    <Badge className={cn("text-[10px] shrink-0", priorityColors[f.priority])}>{f.priority}</Badge>
                    {f.featureType === "ENABLER" && (
                      <Badge className="text-[10px] shrink-0 bg-violet-500/15 text-violet-400">ENABLER</Badge>
                    )}
                    <span className="text-xs text-muted-foreground w-8 text-right shrink-0" title="Business Value">{f.businessValue}</span>
                    <span className="text-xs text-muted-foreground w-8 text-right shrink-0" title="Time Criticality">{f.timeCriticality}</span>
                    <span className="text-xs text-muted-foreground w-8 text-right shrink-0" title="Risk Reduction">{f.riskReduction}</span>
                    <span className="text-xs text-muted-foreground w-8 text-right shrink-0" title="Job Size">{f.jobSize}</span>
                    <span className="text-xs font-bold text-indigo-400 w-10 text-right shrink-0">{computeWsjf(f).toFixed(1)}</span>
                    <span className="text-xs text-muted-foreground w-16 text-right shrink-0">{f.stories.length} stories</span>
                    <Button
                      variant="ghost"
                      size="icon"
                    ...
                    <div className="border-t border-zinc-800 bg-zinc-950/30 px-4 py-3">
                      {f.stories.length === 0 ? (
                        <div className="text-center py-4">
                          <p className="text-xs text-muted-foreground mb-2">No stories yet</p>
                          <Button
                            variant="outline"
                            size="sm"
                            className="gap-1"
                        ...
                        <div className="space-y-1.5">
                          {f.stories.map(s => (
                            <div key={s.id} className="flex items-center gap-3 px-3 py-2 rounded-md bg-zinc-900/50 hover:bg-zinc-900 transition-colors">
                              <Badge className={cn("text-[10px] w-16 justify-center", statusColors[s.status])}>{s.status}</Badge>
                              <span className="text-sm flex-1 truncate">{s.name}</span>
                              {s.team && <span className="text-xs text-muted-foreground">{s.team.name}</span>}
                              {s.iteration && <span className="text-xs text-muted-foreground">{s.iteration.name}</span>}
                              {s.owner && <span className="text-xs text-muted-foreground">{s.owner.user.name}</span>}
                              <span className="text-xs font-mono text-muted-foreground">{s.storyPoints}pt</span>
                              <Button
                                variant="ghost"
                                size="icon"
                                className="h-6 w-6"
