    ...
    <AppShell>
      <div className="space-y-6">
        <div>
          ...
          <h1 className="text-lg md:text-2xl font-bold">Velocity & Predictability</h1>
          <p className="text-xs md:text-sm text-muted-foreground">Team velocity trends and PI predictability metrics</p>
        </div>
          ...
          <Card>
            <CardContent className="p-2 md:p-4">
              <div className="flex items-center gap-1.5 md:gap-2">
                ...
                <p className="text-[10px] md:text-sm text-muted-foreground">ART Velocity</p>
              </div>
          ...
          <Card>
            <CardContent className="p-2 md:p-4">
              <div className="flex items-center gap-1.5 md:gap-2">
                ...
                <p className="text-[10px] md:text-sm text-muted-foreground">Avg Predictability</p>
              </div>
              <p className={cn("text-lg md:text-2xl font-bold mt-1",
          ...
          <Card>
            <CardContent className="p-2 md:p-4">
              <div className="flex items-center gap-1.5 md:gap-2">
                ...
                <p className="text-[10px] md:text-sm text-muted-foreground">Total Committed</p>
              </div>
              <p className="text-lg md:text-2xl font-bold mt-1">{totalCommitted} pts</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-2 md:p-4">
              <div className="flex items-center gap-1.5 md:gap-2">
                ...
                <p className="text-[10px] md:text-sm text-muted-foreground">Total Delivered</p>
              </div>
              <p className="text-lg md:text-2xl font-bold mt-1 text-emerald-500">{totalDone} pts</p>
            </CardContent>
          </Card>
        </div>
        ...
        <Card>
          <CardHeader className="pb-2 md:pb-3">
            <CardTitle className="text-[10px] md:text-sm">Average Velocity by Team (per PI)</CardTitle>
          </CardHeader>
          <CardContent className="p-2 md:p-4">
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={velocityByPI}>
                <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" />
                <XAxis dataKey="pi" tick={{ fill: "#94a3b8", fontSize: 9 }} />
        ...
        <Card>
          <CardHeader className="pb-2 md:pb-3">
            <CardTitle className="text-[10px] md:text-sm">Predictability % by Team (Done / Committed)</CardTitle>
          </CardHeader>
          <CardContent className="p-2 md:p-4">
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={predictabilityByPI}>
                <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" />
                <XAxis dataKey="pi" tick={{ fill: "#94a3b8", fontSize: 9 }} />
                <YAxis domain={[0, 120]} tick={{ fill: "#94a3b8", fontSize: 9 }} />
                <Tooltip
                  contentStyle={{ backgroundColor: "#1e293b", border: "1px solid #334155", borderRadius: "8px", fontSize: 10 }}
                  labelStyle={{ color: "#e2e8f0" }}
          ...
          {sprintVelocityByTeam.map((team) => (
            <Card key={team.name}>
              <CardHeader className="pb-1.5 md:pb-2">
                <div className="flex items-center gap-1.5 md:gap-2">
                  <span className="h-2 w-2 md:h-2.5 md:w-2.5 rounded-full" style={{ backgroundColor: team.color }} />
                  <CardTitle className="text-[10px] md:text-sm">{team.name}</CardTitle>
                </div>
              </CardHeader>
              <CardContent className="p-2 md:p-4">
                <ResponsiveContainer width="100%" height={150}>
                  <LineChart data={team.data}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" />
                    <XAxis dataKey="sprint" tick={{ fill: "#94a3b8", fontSize: 8 }} />
                    <YAxis tick={{ fill: "#94a3b8", fontSize: 8 }} />
                    <Tooltip
                      contentStyle={{ backgroundColor: "#1e293b", border: "1px solid #334155", borderRadius: "8px", fontSize: 9 }}
                      labelStyle={{ color: "#e2e8f0" }}
                    />
        ...
        <Card>
          <CardHeader className="pb-2 md:pb-3">
            <CardTitle className="text-[10px] md:text-sm">PI Summary by Team</CardTitle>
          </CardHeader>
          <CardContent className="p-2 md:p-4">
                  ...
                  {data.teams.map(team => (
                    <tr key={team.id} className="border-b border-border/50">
                      ...
                      {data.pis.map(pi => {
                        ...
                        return (
                          <td key={pi.id} className="text-center py-1.5 px-2 md:py-2 md:px-3">
                            {summary ? (
                              <div className="space-y-0">
                                <div className="font-medium">{summary.avgVelocity} pts/sprint</div>
                                <div className={cn("text-[8px] md:text-[10px]",
